...
export default function ProductsPage() {
  ...
  const { data, loading, error } = useFetch(
    `https://dummyjson.com/products?limit=${ITEMS_PER_PAGE}&skip=${(currentPage - 1) * ITEMS_PER_PAGE}`
  );
  ...
  const sortOptions = useMemo(() => [
    ...
    { value: 'price-low', label: pageTexts.priceLowHigh, path: '/products/sort/price-low' },
    { value: 'price-high', label: pageTexts.priceHighLow, path: '/products/sort/price-high' },
    { value: 'rating', label: pageTexts.rating, path: '/products/sort/rating' }
  ], [pageTexts]);
  ...
  }, [location.search, sortType, sortOptions]);
  ...
  if (loading) {
    return (
      <div className={`min-h-screen w-full py-8 ${isDark ? 'bg-gray-900' : 'bg-gray-800'}`}>
        <div className="max-w-7xl mx-auto px-4">
          {/* Search and Sort Skeleton */}
          <div className="grid gap-4 md:grid-cols-2 mb-6">
            <div className={`h-12 rounded-lg animate-pulse ${isDark ? 'bg-gray-800' : 'bg-gray-700'}`} />
            <div className={`h-12 rounded-lg animate-pulse ${isDark ? 'bg-gray-800' : 'bg-gray-700'}`} />
          </div>

          {/* Stats Skeleton */}
          <div className={`mb-6 p-4 rounded-lg animate-pulse ${isDark ? 'bg-gray-800' : 'bg-gray-700'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[...Array(ITEMS_PER_PAGE)].map((_, i) => (
              <div 
                key={i} 
                className={`rounded-lg overflow-hidden ${isDark ? 'bg-gray-800' : 'bg-gray-700'}`}
              >
  ...
  return (
    <div className={`min-h-screen w-full py-2 sm:py-4 md:py-8 ${isDark ? 'bg-gray-900' : 'bg-gray-100'}`}>
        ...
        <div className="flex flex-col md:flex-row gap-2 sm:gap-4 md:gap-6">
          ...
          <div className="w-full md:w-64 md:flex-shrink-0">
            <div className={`
              ...
              md:sticky md:top-24
            `}>
              <h2 className={`text-base md:text-lg font-semibold mb-2 sm:mb-3 ${
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 md:gap-4 mb-3 sm:mb-4 md:mb-6">
              ...
              <div className="relative">
                ...
                >
                  {sortOptions.find(option => option.value === sortBy)?.label || pageTexts.sortBy}
            ...
            {/* Stats Bar */}
            <div className={`mb-3 sm:mb-4 md:mb-6 p-2 sm:p-3 md:p-4 rounded-lg ${
              isDark ? 'bg-gray-800' : 'bg-white'
            }`}>
